...
var introScreenLegacyAsyncParam = "jam_async";
...
function introScreenShouldAutoConnectLegacyAsync() {
  return new URLSearchParams(window.location.search).get(introScreenLegacyAsyncParam) === "1";
}

function introScreenShowLegacyAsyncBoot() {
  introScreenSetContent([
    '<main class="screen screen-connection" aria-labelledby="legacy-async-title" style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px;box-sizing:border-box;overflow:hidden;">',
    '<h1 id="legacy-async-title" style="max-width:900px;margin:0;color:#f5f1e8;text-align:center;font-family:\'Perfect DOS VGA 437\',monospace;font-size:clamp(24px,5vw,56px);font-weight:700;line-height:1.2;overflow-wrap:anywhere;">',
    'Jam Async connection detected,<br>loading v 1.1',
    '</h1>',
    '</main>'
  ].join(""));
}

function introScreenStartGameWhenReady() {
  if (typeof initialRoomStart === "function") {
    introScreenRoot.remove();
    initialRoomStart();
    return;
  }

  window.setTimeout(introScreenStartGameWhenReady, 50);
}

function introScreenShowConnectionError(introScreenError) {
  introScreenShowConnection().then(function () {
    introScreenRoot.querySelector("#connection-status").textContent = introScreenError.message;
    introScreenShowOnlineFields();
  });
}

function introScreenAutoConnectLegacyAsync() {
  var introScreenConnectionInfo = introScreenLoadSavedConnection();

  introScreenShowLegacyAsyncBoot();
  window.setTimeout(function () {
    archipelagoClientConnect(introScreenConnectionInfo)
      .then(introScreenStartGameWhenReady)
      .catch(function (introScreenError) {
        console.error(introScreenError);
        introScreenShowConnectionError(introScreenError);
      });
  }, 500);
}

function introScreenShowConnection() {
  return shardManagerLoadShard("connection").then(function (introScreenHtml) {
...
function introScreenHandleDownloadGame() {
  ...
  if (introScreenStatus) {
    introScreenStatus.textContent = "Preparing download...";
  }

  downloadManagerDownloadGame().then(function () {
    if (introScreenStatus) {
      introScreenStatus.textContent = "";
    }
  }).catch(function (introScreenError) {
    console.error(introScreenError);
    if (introScreenStatus) {
      introScreenStatus.textContent = "Unable to download shellipelago.zip.";
    }
  });
}
...
if (introScreenShouldAutoConnectLegacyAsync()) {
  introScreenAutoConnectLegacyAsync();
} else {
